...
$result=mysql_query("select modbits
                     from messages
		     where id=$id")
          or sqlbug('ïÛÉÂËÁ SQL ĞÒÉ ×ÙÂÏÒËÅ ÆÌÁÇÏ× ÍÏÄÅÒÉÒÏ×ÁÎÉÑ');
...
mysql_query("update messages
	     ...
	     where id=$id")
  or sqlbug('ïÛÉÂËÁ SQL ĞÒÉ ÕÓÔÁÎÏ×ËÅ ÆÌÁÇÏ× ÍÏÄÅÒÉÒÏ×ÁÎÉÑ');
...
mysql_query("update messages
             set modbits=modbits & ~$bits
	     where id=$id")
  or sqlbug('ïÛÉÂËÁ SQL ĞÒÉ ÓÂÒÏÓÅ ÆÌÁÇÏ× ÍÏÄÅÒÉÒÏ×ÁÎÉÑ');
journal("update messages
         set modbits=modbits & ~$bits
	 where id=".journalVar('messages',$id));
}
...
mysql_query("update messages
             set modbits=$bits
	     where id=$id")
  or sqlbug('ïÛÉÂËÁ SQL ĞÒÉ ÚÁÍÅÎÅ ÆÌÁÇÏ× ÍÏÄÅÒÉÒÏ×ÁÎÉÑ');
journal("update messages
         set modbits=$bits
	 where id=".journalVar('messages',$id));
}
