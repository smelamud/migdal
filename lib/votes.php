...
$ipFilter=$userId<=0 ? 'and votes.ip='.IPToInteger($_SERVER['REMOTE_ADDR']) : '';
$result=sql("select vote
	     from votes
	          left join entries
		       on votes.entry_id=entries.id
	     where entries.parent_id=$id $uidFilter $ipFilter",
	    __FUNCTION__);
return mysql_num_rows($result)>0 ? mysql_result($result,0,0) : 0;
}
...
$parent_id=getParentIdByEntryId($id);
$uid=$userId<=0 ? 0 : $userId;
$ip=$userId<=0 ? IPToInteger($_SERVER['REMOTE_ADDR']) : 0;
$result=sql("insert into votes(entry_id,ip,user_id,vote)
	     values($parent_id,$ip,$uid,$vote)",
	    __FUNCTION__,'insert');
journal("insert into votes(entry_id,ip,user_id,vote)
