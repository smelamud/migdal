...
function answerGet($id) {
    $result = sql("select answers,last_answer,last_answer_id,
                          last_answer_user_id,last_answer_guest_login
                   from entries
                   where id=$id",
                  __FUNCTION__);
...
function answerSet($id, $answers, $last, $last_id, $last_user_id,
                   $last_guest_login, $incVersion = true) {
    if ($id <= 0)
        return;
    sql("update entries
...
function answerFindLastId($id) {
    $result = sql("select id,user_id,guest_login
                   ...
                   limit 1",
                  __FUNCTION__);
    return mysql_num_rows($result) > 0 ? mysql_fetch_array($result)
                                       : array(0, 0);
}
