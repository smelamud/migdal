...
//
// function SDNToJewish($sdn,&$pMonth,&$pDay,&$pYear);
//
...
//
// function JewishToSDN($month,$day,$year);
//
...
define('JEWISH_HALAKIM_PER_HOUR',1080);
define('JEWISH_HALAKIM_PER_DAY',25920);
define('JEWISH_HALAKIM_PER_LUNAR_CYCLE',((29*JEWISH_HALAKIM_PER_DAY)+13753));
define('JEWISH_HALAKIM_PER_METONIC_CYCLE',
       (JEWISH_HALAKIM_PER_LUNAR_CYCLE*(12*19+7)));

define('JEWISH_SDN_OFFSET',347997);
define('JEWISH_NEW_MOON_OF_CREATION',31524);

define('JEWISH_SUNDAY',0);
define('JEWISH_MONDAY',1);
define('JEWISH_TUESDAY',2);
define('JEWISH_WEDNESDAY',3);
define('JEWISH_THURSDAY',4);
define('JEWISH_FRIDAY',5);
define('JEWISH_SATURDAY',6);

define('JEWISH_NOON',(18*JEWISH_HALAKIM_PER_HOUR));
define('JEWISH_AM3_11_20',((9*JEWISH_HALAKIM_PER_HOUR)+204));
define('JEWISH_AM9_32_43',((15*JEWISH_HALAKIM_PER_HOUR)+589));

$jewishMonthsPerYear=array(12,12,13,12,12,13,12,13,12,12,
                           13,12,12,13,12,12,13,12,13);

$jewishYearOffset=array(0,12,24,37,49,61,74,86,99,111,123,
...
$leapYear=$metonicYear==2  || $metonicYear==5  || $metonicYear==7
       ...
       || $metonicYear==18;
$lastWasLeapYear=$metonicYear==3  || $metonicYear==6  || $metonicYear==8
              || $metonicYear==11 || $metonicYear==14 || $metonicYear==17
	      || $metonicYear==0;

// Apply rules 2, 3 and 4.
if(($moladHalakim>=JEWISH_NOON) ||
   ((!$leapYear) && $dow==JEWISH_TUESDAY && $moladHalakim>=JEWISH_AM3_11_20) ||
   ($lastWasLeapYear && $dow==JEWISH_MONDAY && $moladHalakim>=JEWISH_AM9_32_43))
  {
  $tishri1++;
  $dow++;
  if($dow==7)
...
// delay of one day.
if($dow==JEWISH_WEDNESDAY || $dow==JEWISH_FRIDAY || $dow==JEWISH_SUNDAY)
  {
  $tishri1++;
  }
...
//
function MoladOfMetonicCycle($metonicCycle,&$pMoladDay,&$pMoladHalakim)
{
// Start with the time of the first molad after creation.
$r1=JEWISH_NEW_MOON_OF_CREATION;

// Calculate $metonicCycle*JEWISH_HALAKIM_PER_METONIC_CYCLE.  The upper 32
// bits of the result will be in $r2 and the lower 16 bits will be
// in $r1.
$r1+=$metonicCycle*(JEWISH_HALAKIM_PER_METONIC_CYCLE & 0xFFFF);
$r2=$r1 >> 16;
$r2+=$metonicCycle*((JEWISH_HALAKIM_PER_METONIC_CYCLE >> 16) & 0xFFFF);

// Calculate $r2$r1/JEWISH_HALAKIM_PER_DAY.  The remainder will be in $r1, the
// upper 16 bits of the quotient will be in $d2 and the lower 16 bits
// will be in $d1.
$d2=(int)($r2/JEWISH_HALAKIM_PER_DAY);
$r2-=$d2*JEWISH_HALAKIM_PER_DAY;
$r1=($r2 << 16) | ($r1 & 0xFFFF);
$d1=(int)($r1/JEWISH_HALAKIM_PER_DAY);
$r1-=$d1*JEWISH_HALAKIM_PER_DAY;
...
//
function FindTishriMolad($inputDay,&$pMetonicCycle,&$pMetonicYear,
                         &$pMoladDay,&$pMoladHalakim)
{
global $jewishMonthsPerYear;
...
while($moladDay<$inputDay-6940+310)
     {
     $metonicCycle++;
     $moladHalakim+=JEWISH_HALAKIM_PER_METONIC_CYCLE;
     $moladDay+=(int)($moladHalakim/JEWISH_HALAKIM_PER_DAY);
     $moladHalakim=$moladHalakim%JEWISH_HALAKIM_PER_DAY;
     }
...
for($metonicYear=0;$metonicYear<18;$metonicYear++)
   ...
   $moladHalakim+=JEWISH_HALAKIM_PER_LUNAR_CYCLE*
                  $jewishMonthsPerYear[$metonicYear];
   $moladDay+=(int)($moladHalakim/JEWISH_HALAKIM_PER_DAY);
   $moladHalakim=$moladHalakim%JEWISH_HALAKIM_PER_DAY;
   }
...
//
function FindStartOfYear($year,&$pMetonicCycle,&$pMetonicYear,
                         &$pMoladDay,&$pMoladHalakim,&$pTishri1)
{
global $jewishYearOffset;

$pMetonicCycle=(int)(($year-1)/19);
$pMetonicYear=($year-1)%19;
MoladOfMetonicCycle($pMetonicCycle,$pMoladDay,$pMoladHalakim);

$pMoladHalakim+=JEWISH_HALAKIM_PER_LUNAR_CYCLE*$jewishYearOffset[$pMetonicYear];
$pMoladDay+=(int)($pMoladHalakim/JEWISH_HALAKIM_PER_DAY);
$pMoladHalakim=$pMoladHalakim%JEWISH_HALAKIM_PER_DAY;
...
if($inputDay>=$tishri1)
  ...
  $halakim+=JEWISH_HALAKIM_PER_LUNAR_CYCLE*$jewishMonthsPerYear[$metonicYear];
  $day+=(int)($halakim/JEWISH_HALAKIM_PER_DAY);
  $halakim=$halakim%JEWISH_HALAKIM_PER_DAY;
  ...
  if($inputDay>=$tishri1-177)
    ...
    elseif($inputDay>$tishri1-60)
      {
      $pMonth=12;
      $pDay=$inputDay-$tishri1+60;
      }
    elseif($inputDay>$tishri1-89)
      {
      $pMonth=11;
      $pDay=$inputDay-$tishri1+89;
      }
    elseif($inputDay>$tishri1-119)
      {
      $pMonth=10;
      $pDay=$inputDay-$tishri1+119;
      }
    elseif($inputDay>$tishri1-148)
      {
      $pMonth=9;
    ...
    {
    if($jewishMonthsPerYear[($pYear-1)%19]==13)
      {
      $pMonth=7;
...
switch($month)
      ...
      case 6:
           ...
           if($jewishMonthsPerYear[($year-1)%19]==12)
	     {
